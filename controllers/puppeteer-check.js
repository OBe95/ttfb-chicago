...
const checkTimings = async (req, res) => {
  ...
  try {
    ...
    // Calculate the TTFB
    const firstByte = (
      performanceMetrics[0].responseStart - performanceMetrics[0].requestStart
    ).toFixed();

    // Measuring DNS lookup time
    const dnsLookup =
      performanceMetrics[0].connectStart - performanceMetrics[0].fetchStart;

    // Initial connection
    const connectTime =
      performanceMetrics[0].secureConnectionStart -
      performanceMetrics[0].connectStart;

    // SSL handshake
    const sslHandshake =
      performanceMetrics[0].connectEnd -
      performanceMetrics[0].secureConnectionStart;

    // Just a compare that can be deleted
    if (
      performanceMetrics[0].domainLookupEnd ===
      performanceMetrics[0].domainLookupStart
    )
      console.log("Same");
